...
abstract public class AbstractSigningPluginSetup {
    ...
    protected static final String FAKE_PRIVATE_KEY = "-----BEGIN PGP PRIVATE KEY BLOCK-----\n"
            + "\n"
    ...
    protected static final String FAKE_PASSWORD = "This is a fake key";

    /**
     * Helper method to execute test code with in-memory signing environment variables set.
     * Uses System Lambda to temporarily set SIGNING_PRIVATE_KEY and SIGNING_PASSWORD.
     */
    protected void withInMemorySigningEnv(TestAction action) throws Exception {
        withEnvironmentVariable("SIGNING_PRIVATE_KEY", FAKE_PRIVATE_KEY)
                .and("SIGNING_PASSWORD", FAKE_PASSWORD)
                .execute(() -> {
                    action.run();
                    return null;
                });
    ...
    /**
     * Helper method to execute test code with in-memory signing environment variables including subkey ID.
     */
    protected void withInMemorySigningEnvAndSubkey(String subkeyId, TestAction action) throws Exception {
        withEnvironmentVariable("SIGNING_PRIVATE_KEY", FAKE_PRIVATE_KEY)
                .and("SIGNING_PASSWORD", FAKE_PASSWORD)
                .and("SIGNING_SUBKEY_ID", subkeyId)
                .execute(() -> {
                    action.run();
                    return null;
                });
